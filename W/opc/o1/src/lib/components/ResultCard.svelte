...
<div class="animate-scale-in">
  ...
  <div class="relative glass-panel rounded-2xl p-6 sm:p-10 border border-neon-purple/20 max-w-2xl mx-auto text-center">
    ...
    <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
      <button
        class="w-full sm:w-auto px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg font-display text-[11px] sm:text-xs tracking-widest uppercase bg-gradient-to-r from-neon-purple to-neon-pink text-white font-bold shadow-[0_0_20px_rgba(168,85,247,0.3)] hover:shadow-[0_0_35px_rgba(168,85,247,0.5)] transition-all duration-300 active:scale-95"
        onclick={onrestart}
      >
        Restart Mission
      </button>
      <button
        class="w-full sm:w-auto px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg font-display text-[11px] sm:text-xs tracking-widest uppercase glass-panel text-neon-cyan border border-neon-cyan/30 hover:border-neon-cyan/60 hover:shadow-[0_0_20px_rgba(6,245,208,0.2)] transition-all duration-300 active:scale-95"
        onclick={onhome}
      >
        Base Camp
      </button>
